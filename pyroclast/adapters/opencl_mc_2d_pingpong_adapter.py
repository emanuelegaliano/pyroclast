...
This adapter implements the IMonteCarloAdapter Port with a sampling kernel
that keeps the 2-D layout end-to-end through the work-group reduction. The
kernel uses a ping-pong tree reduction over two local scratch buffers (no
in-place writes) in two phases — along x, then along y. The host-side
recursive ``reduce_sum_int`` reducer is shared with the other variants and
collapses the per-work-group partials to a single int.
"""
...
class PyOpenCLMonteCarlo2DPingPongAdapter(PyOpenCLMonteCarloAdapter):
    ...
    def suggest_topology(self, n_runs: int) -> GridTopology:
        """Suggest a 2-D execution grid optimised for the device."""
